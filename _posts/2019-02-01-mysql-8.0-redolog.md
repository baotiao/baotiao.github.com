...
这个线程等待在 log_low 是等待checkpointer_event 被唤醒, 或者超时时间10s 到了, 其实就是pthread_cond_timedwait()

正常情åheckpoint_lsn;

	if (lsn_diff <= log.lsn_capacity) {
  	checkheckpoint_lsn + log.lsn_capacity;
  	break;
	}
	log_request_ch.
}
// 为什么需要在log_writer 的过程加入这个逻辻辑是判断lsn_diff(当前这次要写入的数据的大小) n_capacity(redolog 的剩余容量大小), 如果比它小, 那行写入操作, 就break 出去, 如果比它大, 那么说明后直接影响到线上的写入线程.

```


首先我们必题是, 一次transaction 修改的page 什么时候flush 下å¸ªcheckpoint lsn 点?**

在函数 log_update_available_for_sn_t lwn_lsn = buf_pool_get_oldest_modification_lwm() =>

buf_modification_approx()

这里buf_pool_get_oldest_modification是获得大概的最老的lsn 的位置, 这里是引入了一个问题, 因为引入了 recent_closed buffer 以后, äage 添加到buffer pool 的flush list 是不能保证有序flush list 上面存在的是 98 => 85 => 110 这样的情况.打checkpoint 的, 但是这个lsn 不能保证是最大的嘯否要写这个checkpointer 信息

然后在 log_should_ch¬¡DDL 相关的操作, 到dict_lsn 为止所有的metadata
