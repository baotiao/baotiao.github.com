...
虽然这里做了优化, commit wait 的时候和 disk IO 是并行的, 由于在寄存分离架构下, disk IO 是网络的 disk IO 需要增加网络的延迟, 这里一般单次 IO 在 tcp 场景下是有可嫩需要 300~400us 左右的. 这里可以节省这原本就需要的 300us~400us 左右的等待时间. 但是EC2 TimeSync service 保证的精确时间在 ms 级别.

笔者认为:

由于 EC2 TimeSync service 保证的精确时间在 ms 级别, 虽然做了一些优化, 但是还是远远不够, 性能影响还是有的. 随着分布式事务涉及的节点越来, 这里的时间越差会越大, 可能对性能的影响是越大的.
