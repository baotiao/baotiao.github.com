...
**带来的副作用: page 级的 cleanup lock 竞争**

backend 在只持 Pin、不持 content lock 的状态下继续引用 buffer 里的 tuple 时, vacuum 在同一个 page 上拿不到 cleanup lock (Rule 5 要 refcount = 1). 这个影响是 page 级的, 只影响当前被 pin 的那个 page 的物理整理 (prune / freeze / defrag).

Non-aggressive vacuum 拿不到 cleanup lock 时降级走 lazy_scan_noprune, 跳过这个 page 的 prune / freeze, 不等待:

> A non-aggressive vacuum may choose to skip pruning and freezing if it cannot acquire a cleanup lock on the buffer right away. In this case, it may miss cleaning up dead tuples and their associated index entries (though it is free to reap any existing dead items on the page).

只有需要推进 freeze horizon 的 aggressive vacuum 才必须等 cleanup lock — 它不能跳过待 freeze 的 page (跳了 relfrozenxid 就推进不了).

长事务对 vacuum 的整表影响 (整表 dead tuple 收不掉, bloat 涨) 走的是另一条路径: 老事务的 backend_xmin 卡住 MVCC horizon, dead tuple 在 vacuum 眼里还不算 dead. 这跟 buffer pin 是两件独立的事, 不要混在一起.
...
同时设置 HEAP_XMIN_INVALID 和 HEAP_XMIN_COMMITTED (= HEAP_XMIN_FROZEN) 表示 freeze 一个 tuple. 它用的是 Rule 4 那四个 hint bit 里的两个, 但不走 Rule 4 的 best-effort OR 路径. README 明确说:

> Note, however, that a tuple is frozen by setting both HEAP_XMIN_INVALID and HEAP_XMIN_COMMITTED; this is a critical update and accordingly requires an exclusive buffer lock (and it must also be WAL-logged).

单独 OR 其中一个 bit 是 Rule 4 的 best-effort 写; 两个一起设 (= freeze) 是 critical update, 必须持 exclusive buffer lock + WAL-logged. Freeze 改变的是 tuple 的权威可见性状态, 丢不得, 也不能让 reader 读到中间态.
...
| page 内物理整理 | 单 page 上跟 pin 争 cleanup lock | 不受 reader 阻塞 (X latch 直接拿) |
...
两条路都自洽. 体感差异: PG 里只持 pin 的 reader 会跟 vacuum 在那个 page 上争 cleanup lock, 推迟该 page 的物理整理; InnoDB 的 buf_fix 不挡 btr_page_reorganize, 没有这种情况.
...
这套设计跟 InnoDB 走的是不同路线 — PG 在 buffer manager 这一层多做了 cleanup lock 这套复杂度, 换上层 executor 几乎零拷贝的指针传递; InnoDB 在 buffer manager 这一层保持简单, 把跨原语的位置管理推给 cursor 这一层显式处理. 两套都自洽, 各有代价, 体感上最大的差异是 PG 的 reader 会在 page 级跟 vacuum 争 cleanup lock, InnoDB 没有这种 page 内整理被读者挡住的情况.
